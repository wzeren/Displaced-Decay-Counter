...
void modifyDetectorsSource(const std::string myName, const std::string lookForExpr, const std::string input,
  ...
  if (!fs::exists(inputPath)) throw inputPath.string() + " does not exist.";

  if (!fs::is_regular_file(inputPath)) throw inputPath.string() + " is not a regular file.";

  std::ifstream file(inputPath, std::ios::in);
  if (!file.is_open()) throw inputPath.string() + " cannot be opened.";
...
void modifyDetCutsSource(const std::string myName, const std::string lookForExpr, const std::string input,
                         const std::string fileExtension) {
  fs::path currentPath(fs::current_path());
  fs::path inputPath;
  if (fileExtension.compare("cc") == 0)
...
void CopyNewDet(std::string myName) {
  ...
  /* Open include/CAnalyseEvent.h and go to the line before "std::vector<Detector>
   * CreateDetectors(std::vector<std::string>);" and insert: */
...
int main() {
  try {
    ...
    if (!fs::exists(inputPath)) throw "knowndet.txt does not exist.";

    if (!fs::is_regular_file(inputPath)) throw "knowndet.txt is not a regular file.";
    ...
    while (std::getline(file, str)) {
      if (str.size() > 0) KnownDet.push_back(str);
    }

    std::cout << "List of implemented detectors:" << std::endl;
    for (auto str : KnownDet) std::cout << str << std::endl;

    std::string DetName;
    std::cout << "Please enter the name of the new detector:" << '\n';  // Prompt.
    std::getline(std::cin, DetName);

    auto itKnownDet = std::find_if(KnownDet.begin(), KnownDet.end(),
                                   [&DetName](std::string name) { return name.compare(DetName) == 0; });
