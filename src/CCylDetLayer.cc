 ...
 * The straightforward constructor of the CylDetLayer class directly takes a vector of oriented cylindrical segments and
 a weight as input. We do not encourage its use.
*/
 ...
 * The 'advanced' constructor reads a list of coordinates (a vector of 2-dim. arrays) in the cylindrical plane {z,h},
 together with the weighing factor. It then builds the convex polygonal envelope associated with the coordinates,
      ...
      for (int j = i + 1; j < ptlist.size() - 1; j++) {
        if (abs(ptlist[j][0] - ptlist[i][0]) > 1e-5 || abs(ptlist[j][1] - ptlist[i][1]) > 1e-5) {
          for (int k = j + 1; k < ptlist.size(); k++) {
            detcomp = abs((ptlist[j][1] - ptlist[i][1]) * (ptlist[k][0] - ptlist[i][0]) -
                          (ptlist[j][0] - ptlist[i][0]) * (ptlist[k][1] - ptlist[i][1]));
  ...
  if (nonalign) {
    for (int i = 0; i < ptlist.size(); i++) {
      if (abs(ptlist[i][0] * ptlist[i][0] + ptlist[i][1] * ptlist[i][1]) > 1e-10) {
        ...
        if (th == thmax && ptlist[i][0] * ptlist[i][0] + ptlist[i][1] * ptlist[i][1] >
                               ptlist[imax][0] * ptlist[imax][0] + ptlist[imax][1] * ptlist[imax][1]) {
          thmax = th;
          imax = i;
        }
        if (th < thmin) {
        ...
        if (th == thmin && ptlist[i][0] * ptlist[i][0] + ptlist[i][1] * ptlist[i][1] <
                               ptlist[imin][0] * ptlist[imin][0] + ptlist[imin][1] * ptlist[imin][1]) {
          thmin = th;
          imin = i;
        }
      }
  ...
  if (nonalign) {
    ...
    do {
      ...
      for (int i = 0; i < ptlist.size(); i++) {
        bool conv = true;
        if (i == istart ||
            (abs(ptlist[istart][0] - ptlist[i][0]) < 1e-5 && abs(ptlist[istart][1] - ptlist[i][1]) < 1e-5))
          conv = false;
        for (int j = 0; j < ptlist.size(); j++) {
          if (!conv) break;
          if (j == istart || j == i) continue;
          detcomp = (ptlist[istart][0] - ptlist[i][0]) * (ptlist[j][1] - ptlist[i][1]) -
                    (ptlist[istart][1] - ptlist[i][1]) * (ptlist[j][0] - ptlist[i][0]);
          if (detcomp < -1e-10) conv = false;
          if (abs(detcomp) < 1e-10 && (ptlist[istart][0] - ptlist[i][0]) * (ptlist[istart][0] - ptlist[i][0]) +
                                              (ptlist[istart][1] - ptlist[i][1]) * (ptlist[istart][1] - ptlist[i][1]) <
                                          (ptlist[istart][0] - ptlist[j][0]) * (ptlist[istart][0] - ptlist[j][0]) +
                                              (ptlist[istart][1] - ptlist[j][1]) * (ptlist[istart][1] - ptlist[j][1]))
            conv = false;
        }
