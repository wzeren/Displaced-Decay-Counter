...
Detector DELIGHTB() {
    ...
    double x_max_overall = 125.0;
    double y_min_overall = 0.0;
    double y_max_overall = 100.0;

    std::vector<CylDetLayer> delightLayers;

    // Tiling in x and y 
    int num_x_slices = 50; // number of slices in x
    int num_y_slices = 50; // number of slices in y

    double x_dim = x_max_overall - x_min_overall; // 100 m
