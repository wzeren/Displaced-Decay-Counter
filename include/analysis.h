...
class analysis {
 public:
  ...
  void setDETECTORS(std::vector<std::tuple<std::string, double>> myDetectorList_in) {
    myDetectorList = myDetectorList_in;
  };
 ...
 private:
  ...
  bool isLast_hepmc(HepMC::GenEvent::particle_const_iterator p,
                    int PID);  // input: HepMC particle and PID, output: check if the input is the last particle that
                               // "decays to itself"
};
#endif
